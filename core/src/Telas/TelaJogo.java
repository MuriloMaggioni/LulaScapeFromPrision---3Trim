...
import Ferramentas.B2MapaCriacao;
import Ferramentas.InterpretadorContatoMapa;
import Scenes.Hud;
...
public class TelaJogo implements Screen {
    ...
    public TelaJogo(LulaScape game) {
        ...
        creator = new B2MapaCriacao(this);

        //cria o personagem no jogo
        jogador = new Personagem(this);

        mapa.setContactListener(new InterpretadorContatoMapa());
    ...
    public void criandoItens(){
        if (!itensSpawn.isEmpty()){
            ...
            if (id.tipo == Powerup.class){
                itens.add(new Powerup(this, id.posicao.x, id.posicao.y));
            }
        }
    }
    ...
    public void apertaTecla(float tempo) {
        if (jogador.estadoAtual != Personagem.State.MORTO) {
            if (Gdx.input.isKeyJustPressed(Input.Keys.UP)) {
                if (jogador.estadoAtual != Personagem.State.PULANDO){
    ...
    public void atualiza(float tempo){
        ...
        //acorda os inimigos
        for (Inimigo inimigo : creator.getInimigos()){
    ...
    public void render(float delta) {
        ...
        for (Inimigo i : creator.getInimigos())
            i.draw(game.batch);
    ...
    public void gravaPontos() {
        FileHandle fh = Gdx.files.local("pontosFase1.txt");
    ...
    public boolean termina(){
        if (jogador.estadoAtual == Personagem.State.GANHOU &&
                jogador.getTempoEstado() > 3){
            return true;
        }
        return false;
